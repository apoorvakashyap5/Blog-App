...
export default function PostsList() {
 ...
 const category = useSelector(state => state?.category);
  ...
  useEffect(() => {
    dispatch(fetchPostsAction("")); //empty string means all posts
  }, [dispatch,likes , dislikes]); //whenever likes/dislikes is done, rerender post, so that we dont have to refresh to see the likes/dislike
            ...
            <div className="flex flex-wrap -mx-3">
              <div className="mb-8 lg:mb-0 w-full lg:w-1/4 px-3">
                <div className="py-4 px-6 bg-gray-600 shadow rounded">
                  ...
                  <ul>
                   {catLoading?<h1>Loading..</h1>: catAppErr|| catServerErr? <h1>{catServerErr} - {catAppErr}</h1>: categoryList?.length<=0?<h1> No categories found</h1> :categoryList?.map((category)=>(<li>
                      ...
                      <div className="mb-10  w-full lg:w-1/4 px-3">
                        ...
                        <div className="flex flex-row bg-gray-300 justify-center w-full  items-center ">
                          ...
                          <div className="flex flex-row justify-center items-center ml-4 mr-4 pb-2 pt-1">
                            {/* Togle like  */}
                            <div className="">
                              <ThumbUpIcon onClick={()=>dispatch(toggleAddLikesToPost(post?._id))} className="h-7 w-7 text-indigo-600 cursor-pointer hover:bg-green-600 active:bg-pink-600" />
                            </div>
                          ...
                          <div className="flex flex-row  justify-center items-center ml-4 mr-4 pb-2 pt-1">
                            <div>
                              <ThumbDownIcon onClick={()=>dispatch(toggleAddDisLikesToPost(post?._id))} className="h-7 w-7 cursor-pointer text-gray-600 hover:bg-red-500 active:bg-purple-800" />
                            </div>
                            <div className="pl-2 text-gray-600">
